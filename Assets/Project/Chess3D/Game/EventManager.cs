        ...
        if (Input.GetMouseButtonDown(0) && isEnableInput)
        {
            ...
            else if (gc.SelectedPiece != null)
            {
                ...
                if (gc.IsValidCell(sq64))
                {
                    ...
                    {
                        Debug.Log("I am king");
                        Debug.Log(Spawner.taken_piece);
                        if(Spawner.taken_piece == 1){
                            PieceTaken piece_taken = new PieceTaken(Splash.client_username, Spawner.piece_label);
                            socket.Emit("piece_taken", JsonUtility.ToJson(piece_taken));    
                            Spawner.taken_piece = 0;
                        }
                        socket.Emit("click", JsonUtility.ToJson(playerTurn));
...
public class PieceTaken
{
    ...
    public char piece_label;//  piece label
     public PieceTaken(string name, char label)
    {
        this.name = name;
        this.piece_label = label;
    }
}
